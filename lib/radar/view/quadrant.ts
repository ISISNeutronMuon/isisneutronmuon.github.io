...
export class Quadrant {
  ...
  labelPosition(outerRingRadius: number): {
    x: number;
    y: number;
    ...
    return {
      x: outerRingRadius * this.radialBasis.x,
      y: outerRingRadius * this.radialBasis.y,
      anchor: anchor,
    };
  }
  ...
  private init(quadrantPos: string) {
    switch (quadrantPos) {
      case "top-left": {
        ...
        this.linkRectOriginFn = (radius: number) => {
      ...
      case "bottom-right": {
        ...
        this.linkRectOriginFn = (_: number) => {
          return { x: 0, y: 0 };
        };
        break;
      }
      ...
      default:
        throw Error(`Unknown quadrant position id: ${quadrantPos}`);
    }
  }
}
